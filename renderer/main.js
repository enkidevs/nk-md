...
vmd.onContent(function (ev, data) {
  ...
  try {
    ...
    insight.topic = {}
    insight.subtopic = {name: ''}
    insight.category = {
      name: insight.category
    }

    if (data.filePath) {
      const names = enkiContent.extractNames(data.filePath)
      const topic = yaml.safeLoad(data.topic) || {}
      topic.name = names.topicName
      topic.color = '#' + topic.color
      insight.topic = topic
      insight.subtopic.name = names.subtopicName
    }
...
vmd.onToggleRevision(function (ev) {
  showRevision = !showRevision
  if (window.insight) {
    console.log(ev)
    console.log(showRevision)
    window.renderInsight(window.insight, showRevision)
  }
})
